...
1. 模型有预训练这个过程，如果用更多样化的数据集去训练会不会最后的效果更好？
灵感来源:[如何打造一个文档解析的多模态大模型？MinerU2.5架构、数据、训练方法](https://mp.weixin.qq.com/s?__biz=Mzg4NjI0NDg0Ng==&mid=2247489820&idx=1&sn=8a4ec392ceca728b2d6925b0433391c1&chksm=cf9dcd9df8ea448baa2598a50ed588c0a14cf8e8744898a7ae7e2772468b63a632027bcd34fd&scene=178&cur_album_id=3027406546135171084&search_click_id=#rd)
1. 表格合并这里，为什么那个内蒙古那个pdf的表格时而会合并，时而不会，跟前面布局分析的时候对表格的覆盖有关，那些没有覆盖表格标题的（表格标题是在框线内的，却把表格标题识别为是段落文本了），都不会被识别成和前面是一个表格。那些合并的，都是因为识别到表格标题是一样的，于是给合并了。
## 四、学习
### 4.1 涉及的模型学习
#### 4.1.1 pipeline
