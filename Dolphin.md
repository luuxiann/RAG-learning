...
![S](./pictures/S6.jpg)
3. 具体：随着网络层数的加深，输入的特征图会逐渐减小其空间尺寸（即 H 和 W 维度），而同时增加其通道数（即 C 维度），以便模型可以捕捉到更为复杂的高层次信息。
   假设输入的特征图形状为 H x W x C，`PatchMerging`：
   * 分割和拼接（Splitting and Concatenation）：
       * 输入的特征图按照一定的步长（通常是 2）进行分割，即对每个 2x2 的 patch 进行合并。
...
#### 2.7 分层计算
![S](./pictures/S1.jpg)
### Swin Transformer“分层计算”的核心总结
Swin Transformer的分层计算是 **“下采样降维+通道扩增+注意力特征抽象”的逐层递进流程**，核心是通过`Patch Merging`与注意力模块的配合，实现 **“效率可控+特征逐步复杂”** 的视觉特征提取，可概括为3个关键：

1. **分层下采样：控制计算复杂度**
   每经过一个“Stage”，通过`Patch Merging`将特征图的**空间尺寸减半**（比如从$\frac{H}{4}×\frac{W}{4}$→$\frac{H}{8}×\frac{W}{8}$），本质是将2×2的相邻token合并为1个，既减少了后续注意力计算的token数量，又让模型的“感受野”（特征捕捉范围）同步扩大。

2. **通道逐层扩增：提升特征丰富度**
   下采样的同时，特征图的**通道数翻倍**（比如从$C$→$2C$）——合并后的token需要更多通道来编码更复杂的特征信息，通过线性变换（或卷积）实现“4个token的特征→2倍通道的新特征”，平衡了特征丰富度与通道膨胀问题。

3. **特征逐层抽象：适配视觉任务需求**
   前序Stage（如Stage 1-2）在小感受野、低通道数下提取**局部细节特征**（如边缘、纹理）；后序Stage（如Stage 3-4）在大感受野、高通道数下提取**全局语义特征**（如物体轮廓、类别信息），符合图像任务“从细到粗”的特征学习逻辑。


### 分层计算的核心作用
- 既借鉴了CNN“卷积+池化”的分层降维逻辑，解决了传统Transformer处理高分辨率图像时的**计算效率问题**；
- 又保留了Transformer的注意力机制优势，通过“通道扩增+逐层抽象”实现了视觉特征的**逐步复杂与全局关联**。

简单说：分层计算是Swin Transformer“适配视觉任务”的关键设计——用下采样控效率，用通道扩增提特征，用注意力做抽象，最终平衡了效率与效果。
